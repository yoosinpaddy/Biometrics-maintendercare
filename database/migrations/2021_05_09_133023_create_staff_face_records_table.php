...
class CreateStaffFaceRecordsTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('staff_face_records', function (Blueprint $table) {
            $table->id();
            $table->string('reg_no');
            $table->foreign('reg_no')->references('staff_id')->on('staff');//eno
            ...
            $table->string('staff_type')->default('teaching');//teaching non-teaching
    ...
    {
        Schema::dropIfExists('staff_face_records');
    }
}
